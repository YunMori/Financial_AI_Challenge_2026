...
→ **`eval/recalibrate.py` 신설.** 리포트를 받아 정상 문항 vs `no_evidence` 문항의
   `top1` 분포에서 언어별 임계값을 재산출하고, 그 임계값으로 게이트 의존 지표를
   오프라인 재계산한다. 후보를 같은 자에 놓는 유일한 방법이다.
   **입력 리포트의 `backend`·`device` 가 섞이면 거부한다**(§6.1).
