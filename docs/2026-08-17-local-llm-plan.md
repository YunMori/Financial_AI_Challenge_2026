...
- 상태: **계획 확정 / 미착수**
- ⚠ **장치 전제가 바뀌었다 — [ADR-005](./adr/ADR-005-cuda-only-dev.md) (2026-08-19).**
  이 문서의 mps 관련 서술(스크리닝/판정 분리, AWS 판정 전제)은 **당시 기록**이며
  현재는 개발·판정 모두 로컬 CUDA(RTX 5060 Ti 16GB) bf16 단일 경로다.
...
**② int8 은 정규화 품질에 저하가 없다.** `run_eval.py --no-generation` 을 신설해
(③ 만 돌리고 ⑦ 은 건너뛴다 — 18시간이 17분이 된다) vi 41문항을 쟀다:
Recall@5 **fp16 90.5% (19/21) vs int8 100% (21/21)**. 모수가 21이라 2문항 차이이므로
"더 낫다"로 읽지 않고 **"무너지지 않았다"**로만 읽는다.

**지연은 MPS 에서 8.7배 느려 판정할 수 없다.** 크기 목표(−42%)와 배선 검사
