...
### 4.3 ★ int8 정규화 품질 — 저하 없음 (vi 41문항, `--no-generation`)

**⑦ 생성을 건너뛰면 MPS 에서도 잴 수 있다.** Recall@5 는 ③ 정규화에만 의존하고
(검색이 정규화된 검색어로 돈다) ⑦ 의 산출물을 쓰지 않는데, 두 단계의 비용이
자릿수로 다르기 때문이다:

    ③ 정규화만   ~25초 × 41문항 ≈ 17분      ← 잴 수 있다
    ⑦ 포함      ~1600초 × 41   ≈ 18시간     ← 못 잰다

그래서 `eval/run_eval.py` 에 **`--no-generation`** 을 넣었다. `--no-llm` 으로는
안 되는데, `NullLLMClient` 는 번역기까지 꺼서(정규화도 LLM 호출이다) 재려는
대상 자체가 사라지기 때문이다.

| | Recall@5 | ③ 지연 p50 | 과잉폴백 | 폴백정확 |
|---|---:|---:|---:|---:|
| textonly (fp16) | 90.5% (19/21) | 1,537 ms | 4.8% | 65% |
| **textonly-int8** | **100% (21/21)** | 7,673 ms | 14.3% | 65% |

**결론: int8 이 ③ 정규화를 망가뜨리지 않는다.** 이것이 이 측정에서 말할 수 있는
전부다.

⚠ **"int8 이 더 낫다"고 읽으면 안 된다.** 세 가지 이유가 있다:

1. **모수가 21문항이다.** 90.5% → 100% 는 **2문항 차이**다. 양자화는 로짓을
   흔들어 토큰 선택을 양방향으로 뒤집으므로, 좋아진 쪽으로 2건이 몰리는 것은
   개선이 아니라 잡음으로 보는 것이 맞다.
2. **과잉폴백은 반대로 나빠졌다**(4.8% → 14.3%, 1건 → 3건). 다만 이 지표는
   임계값 의존이라 계획서 §5 에 따라 후보별 재보정 없이는 비교할 수 없다 —
   리포트도 `gate_metrics_comparable: false` 로 표시된다.
3. **판정은 CUDA 다**(계획서 §8). 이 수치는 MPS fp16 대 MPS int8 이다.

그럼에도 값이 있다: **AWS 를 빌리기 전에 "int8 이 최약 고리를 무너뜨리지는
않는다"를 17분에 확인했다.** 계획서 §7.9 가 vi 를 고른 이유가 그 축이었다.
무너졌다면 GPU 요금을 쓰기 전에 알았을 것이다.

리포트: `eval/reports/exp_009_qwen35-4b-textonly{,-int8}_mps_nogen.json`

### 4.4 vi 41문항 전량(⑦ 포함) — 돌리지 않는다
...
**int8 — ⑦ 를 포함하면 이 장치에서 돌릴 수 없다.** 41문항이 **18시간 규모**다.
계획서 §7.9 가 잡은 "30분~1시간"과 자릿수가 다르다. ③ 만 따로 잰 것이 §4.3 이다.
...
- **CUDA int8** — 이것이 본선이다. 기대의 근거는 "int8 텐서코어"가 **아니다**:
  weight-only 는 가중치만 int8 이고 활성값은 bf16 이라 행렬곱은 bf16 텐서코어로
  돈다. 이득은 연산이 아니라 **메모리 대역폭**(읽을 바이트 절반)이고, 배치 1
  자기회귀 디코딩이 대역폭 병목이라 여기서 난다. MPS 가 느린 것도 그 융합 커널이
  없어 매 스텝 fp16 사본을 만들기 때문으로 보인다.
  (활성값까지 int8 로 내려 텐서코어를 쓰려면 `Int8DynamicActivationInt8WeightConfig`
  가 필요하고, 그건 별개의 실험이다.)
- **24GB GPU 에서 int8 의 값이 무엇인지 먼저 정할 것.** 계획서 §3 표에서
  Qwen3.5-4B + KV ≈ 11GB 로 bf16 그대로 "여유"다. 즉 거기서 int8 은 "올릴 수
  있느냐"가 아니라 지연·비용의 문제다. 더 강한 동기는 `bench_local_speed.py` 가
  적어 둔 **$100 예산의 CPU 배포**(월 ~$720 vs ~$30)이고, 그건 **CPU int8** 을
  재야 답이 나온다.
...
크기 목표(8.43 → 4.87GB, −42%)는 달성했고, 배선·문법·필드 채움도 전부 통과했고,
**③ 정규화 품질도 저하가 없다**(§4.3). **MPS 에 int8 커널이 없다는 장치 문제**로
지연을 판정할 수 없을 뿐이다.
