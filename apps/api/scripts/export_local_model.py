...
실측 결과 (2026-08-18 · MPS · M3 Pro 18GB) — **폐기된 장치의 기록이다**
------------------------------------------------------------------------
ADR-005 로 개발·판정이 cuda 단일 경로가 되었다. 아래 표는 그 이전 측정이며,
디스크·로드 수치는 장치와 무관해 유효하지만 **③ 정규화 지연은 무효다.**

    산출물          디스크    로드    ③ 정규화   결정
    원본            9.32 GB   28초    2.4초     기준선
    textonly        8.43 GB   17초    2.4초     ★ 채택 — 로짓 최대차 0, 출력 동일
    textonly-int8   4.87 GB   15초   20.8초     ← MPS 아티팩트. cuda 재측정 대상

★ **①의 이득은 런타임 메모리가 아니다.** `AutoModelForCausalLM` 은 원본에서도
  텍스트 스택만 올린다(양쪽 다 4841.5M params · visual=False). 남는 것은
  디스크·콜드스타트 읽기량이다.

★ **②의 8.7배 지연은 MPS 탓이었다.** 배선·문법·필드 채움은 전부 통과했으므로
  품질이 아니라 장치 문제였다 — MPS 에 int8 행렬곱 커널이 없어 매 연산마다
  fp16 으로 역양자화했다. cuda 는 텐서코어가 있어 뒤집힐 가능성이 크고,
  **4.87GB 는 16GB VRAM 에 1차 3종을 모두 올릴 여지를 만든다** — bf16 으로는
  4B 하나뿐이다. ADR-005 의 후속 작업에서 재측정한다.
