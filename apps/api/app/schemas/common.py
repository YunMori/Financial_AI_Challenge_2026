...
class Lang(StrEnum):
    """공개 언어. planner 가 상정한 6종을 전부 연다 (2026-08-21).

    ★★ **§10.3 "검수되지 않은 언어는 공개하지 않는다"와 정면으로 부딪히는
       결정이다.** 그래서 언어를 여는 것과 **검수됐다고 말하는 것**을 분리했다 —
       `REVIEWED` 에 없는 언어는 화면 상단에 미검수 고지가 상시로 붙고, 원문
       기준(한국어)을 함께 안내한다.

       숨기는 것보다 낫다고 판단한 근거: 대상 이용자는 한국어를 못 읽는다.
       "준비 중"으로 막아 두면 그 이용자에게 이 서비스는 **존재하지 않는 것**과
       같다. 번역이 있고 그 한계를 밝히는 쪽이, 없는 것보다 이용자에게 낫다.

       ⚠ 다만 **금액·서류명·법정 한도처럼 오역이 치명적인 항목은 여전히 번역에
       맡기지 않는다.** 용어는 `corpus/glossary/glossary.csv` 가 고정하고,
       숫자는 §8.3 숫자 대조가 근거 텍스트와 대조한다. 미검수 언어에서 무너지는
       것은 **문장의 자연스러움**이지 값의 정확성이 아니다.

       → 원어민 검수를 확보하는 대로 `REVIEWED` 에 추가한다(planner §0.1 0-8).
    """
...
#: 원어민 검수를 마친 언어. **여기 없으면 화면에 미검수 고지가 붙는다.**
#: ko 는 원문이고, en·vi 는 M1 에서 검수 경로를 거쳤다(dev-log 2026-08-12 조판 검증 포함).
REVIEWED: frozenset[Lang] = frozenset({Lang.KO, Lang.EN, Lang.VI})
